...
class PCBADetectionSystem(QMainWindow):
    ...
    def closeEvent(self, event): 
        ...
        try:
            ...
            print(f"關閉程序時發生錯誤: {e}")
            event.accept()  # 確保程序能夠關閉
    
    def stop_threads_safely(self):
        """安全停止所有線程"""
        threads = [
            (self.camera_thread, "相機線程"),
            (self.detection_thread, "檢測線程")
        ]
        ...
        for thread, name in threads:
            if thread and thread.isRunning():
                ...
                if not thread.wait(3000):  # 等待3秒
                    print(f"強制終止{name}")
                    thread.terminate()  # 強制終止
                    thread.wait(1000)   # 等待1秒確保終止
                else:
                    print(f"{name}已安全停止")

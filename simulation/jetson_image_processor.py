"""
Jetson Orin Nano 最佳化版影像處理和YOLOv12推論模組
重點針對JetPack 6.2環境進行效能最佳化
"""
            ...
            if temps:
                for name, entries in temps.items():
                    for entry in entries:
                        if entry.current:
                            print(f"溫度 ({name}): {entry.current:.1f}°C")
        except:
            print("溫度: 無法取得")
            
    except ImportError:
        print("⚠️ 需要安裝 psutil: pip install psutil")

def test_jetson_processor():
    """測試Jetson影像處理器"""
    print("🧪 測試Jetson影像處理器...")
    ...
    print(f"📊 Jetson統計: FPS={stats['fps']:.1f}, CUDA={stats['cuda_available']}")

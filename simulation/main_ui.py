...
class PCBADetectionSystem(QMainWindow):
    ...
    def create_status_widget(self):
        ...
        self.camera_status = QLabel("相機狀態：🟢 正常")
    ...
    def on_sensor_triggered(self):
        """感測器觸發事件"""
        self.sensor_status.setText("光電感測器：🟡 檢測中...")
        QTimer.singleShot(1000, lambda: self.sensor_status.setText("光電感測器：🟢 正常"))
    ...
    def update_sensor_status(self):
        ...
        if hasattr(self, 'hardware'):
            ...
            if not self.is_running:
                sensor_state = self.hardware.read_sensor()
                if sensor_state:
                    self.sensor_status.setText("光電感測器：🟡 檢測到物體")
                else:
                    self.sensor_status.setText("光電感測器：🟢 正常")
    ...
    def update_status_displays(self):
        ...
        if hasattr(self.hardware, 'robotic_arm') and self.hardware.robotic_arm:
            ...
            angle = getattr(self, 'servo_slider', {}).get('value', lambda: 90)()
